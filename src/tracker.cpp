...
float Tracker::CalculateL2(const cv::Rect &det, const Track &track) {
  const auto trk{track.GetStateAsBbox()};

  const float x0{static_cast<float>(trk.x + (trk.width >> 1))};
  const float y0{static_cast<float>(trk.y + (trk.height >> 1))};
  const float x1{static_cast<float>(det.x + (det.width >> 1))};
  const float y1{static_cast<float>(det.y + (det.height >> 1))};
...
void Tracker::AssociateDetectionsToTrackers(
    ...
    float iou_threshold, float l2_threshold) {
  ...
  for (size_t i = 0; i < detection.size(); i++) {
    ...
    for (const auto &trk : tracks) {
      iou_matrix[i][j] = -CalculateL2(detection[i], trk.second);
  ...
  for (size_t i = 0; i < detection.size(); i++) {
    ...
    for (const auto &trk : tracks) {
      if (0 == association[i][j]) {
        ...
        if (-iou_matrix[i][j] <= l2_threshold) {
