...
public:
  ...
  static float CalculateL2(const cv::Rect &det, const Track &track);
  static float CalculateIou(const cv::Rect &det, const Track &track);

  static void
  HungarianMatching(const std::vector<std::vector<float>> &iou_matrix,
  ...
  static void AssociateDetectionsToTrackers(
      ...
      float iou_threshold = 0.3, float l2_threshold = 150.0f);
